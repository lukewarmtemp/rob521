...
# various np seeds used in testing!
np.random.seed(42)
# np.random.seed(19)
# np.random.seed(42)
...
# parameters for how big to make the plot points
markers = 1
line = 0.5

# plots trajectory with a single point (testing simulate_trajectory)
...
# same thing with no point
def plot_trajectory_with_orientation2(trajectory):
    for i in range(trajectory.shape[1]):
        x, y, theta = trajectory[:, i]
        plt.plot(x, y, 'bo') 
        line_length = 2
        dx = line_length * np.cos(theta)
        dy = line_length * np.sin(theta)
        plt.plot([x, x + dx*0.1], [y, y + dy*0.1], 'r-', lw=1)
    plt.grid(True)
    plt.axis("equal")
    plt.show()

# displays the end of rrt or rrt star with the map and all params
def showing_nodes_and_background_star(array, nodes, closest_node, point, goal):
    plt.imshow(array, cmap='gray', origin='upper')
    print("num nodes:", len(nodes))
    for node in nodes:
        x, y = node.point[:2].flatten() 
        parent_node = nodes[node.parent_id]
        parx, pary = parent_node.point[:2].flatten()
        plt.plot(x, y, 'bo', markersize=markers)
        if node.parent_id != -1:
            plt.plot([parx, x], [pary, y], 'b-', linewidth=line)
    ...
    plt.plot(closest_x, closest_y, 'go', markersize=markers, label="Closest Node")

    point_x, point_y = point
    plt.plot(point_x, point_y, 'mo', markersize=markers, label="Target Point")

    goal_x, goal_y = goal.flatten()
    plt.plot(goal_x, goal_y, 'ko', markersize=markers, label="Goal Point")

    plt.xticks([])
    plt.yticks([])
    plt.show()

# displays the path only with the background image
def plot_path_with_background(background_image, path):
    if isinstance(background_image, str): bg_img = mpimg.imread(background_image)
    else: bg_img = background_image
    x_points = path[0, :]
    y_points = path[1, :]
    plt.figure()
    plt.imshow(bg_img, cmap='gray', origin='upper')
    plt.plot(x_points, y_points, 'b-o', markersize=markers, linewidth=line)
    plt.xticks([])
    plt.yticks([])
    plt.show()
...
# info for each individual node to store trajectory and cost and other info
class Node:
    def __init__(self, point, parent_id, cost):
        ...
        self.area = np.zeros((2, 1))        # Holds coordinates of its surrounding neighbours
        return

# main class that inits everything for the path planner
class PathPlanner:
    ...
    def __init__(self, map_filename, map_setings_filename, goal_pix, first_node, stopping_dist):
        ...
        self.pix_vel = 3            # pixels
        self.pix_rot_vel = 0.1      # rad/s
        ...
        # Bound the image depending on the image and choose ratios for guided sampling
        if "willowgarageworld" in map_filename:
            zero_indices = np.argwhere(self.occupancy_map == 0)
            self.min_y, self.min_x = zero_indices.min(axis=0)  # first 0 bound
            self.max_y, self.max_x = zero_indices.max(axis=0)  # last 0 bound
    ...
    def cell_to_point(self, cells):
        ...
        converted_points = np.zeros((3, cells.shape[1]))
        converted_points[0] = cells[0] * res + x_0
        converted_points[1] = (y_size - cells[1]) * res + y_0
        # calculate the angles in radians
        for i in range(cells.shape[1] - 1):
            dx = converted_points[0, i + 1] - converted_points[0, i]
            dy = converted_points[1, i + 1] - converted_points[1, i]
            converted_points[2, i] = np.arctan2(dy, dx)
        # maintain the last angle as the same as the second last angle
        if cells.shape[1] > 1: converted_points[2, -1] = converted_points[2, -2]
        return converted_points
    ...
    def sample_map_space(self):
        ...
        # mini function to just sample a point
        ...
        # sampling idea: change the ratios of what type of sampling strategy!
    ...
    def rrt_planning(self):
        ...
        while True:
            ...
            # increase the iterations (this is only for display purposes)
            iterations +=1 
    ...
    # until all nodes are consistent, we continue updating node paths
    ...
    def rrt_star_planning(self):
        ...
        # initialise params for anytime goal
        goal_id = None
        min_iterations = 100 
...
def main():
    ########################################################
    # CHOOSE FROM THIS SECTION ONE THING TO UNCOMMENT
    ...
    # stopping_dist = 0.5 #m
    # rrt_path = "/Users/felicialiu/Desktop/DEV/ROB521_MobileRobotics/LAB2/lab2/maps/simple_map_coords_ori.npy"
    # rrt_star_path = "/Users/felicialiu/Desktop/DEV/ROB521_MobileRobotics/LAB2/lab2/maps/simple_map_rrtstar_coords_ori.npy"
    ...
    # stopping_dist = 0.5 #m
    # rrt_path = "/Users/felicialiu/Desktop/DEV/ROB521_MobileRobotics/LAB2/lab2/maps/willowgarageworld_05res_coords_ori.npy"
    # rrt_star_path = "/Users/felicialiu/Desktop/DEV/ROB521_MobileRobotics/LAB2/lab2/maps/willowgarageworld_05res_rrtstar_coords_ori.npy"

    # map information (random seed 42)
    map_filename = "myhal.png"
    map_setings_filename = "myhal.yaml"
    # robot information
    goal_pix = np.array([[152], [40]])
    first_node = Node(np.array([[8],[40],[0]]), -1, 0)
    stopping_dist = 0.2 #m
    rrt_path = "/Users/felicialiu/Desktop/DEV/ROB521_MobileRobotics/LAB2/lab2/maps/myhal_newgoal_coords_ori.npy"
    rrt_star_path = "/Users/felicialiu/Desktop/DEV/ROB521_MobileRobotics/LAB2/lab2/maps/myhal_newgoal_rrtstar_coords_ori.npy"


    ########################################################
    # SETS UP THE RRT/RRT* CLASS OBJECT

    path_planner = PathPlanner(map_filename, map_setings_filename, goal_pix, first_node, stopping_dist)

    ########################################################
    # RUNS RRT AND EXTRACT/SAVE THE PATH
    ...
    ########################################################
    # AFTERWARDS MANUALLY CONVERT THE PATHS BACK INTO COORDINATE FORM

    # path_name = rrt_path
    # cells = np.load(path_name)
    # points = path_planner.cell_to_point(cells)
    # np.save(path_name.replace(".npy", "_coords_ori.npy"), points)
    ...
    ########################################################
    # PLOTS COORDINATE PATHS AFTERWARDS TO VERIFY CORRECTNESS
    ...
    # plot_trajectory_with_orientation2(np.load(rrt_path))
    # plot_trajectory_with_orientation2(np.load(rrt_star_path))
